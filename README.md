...
2. If a module is already deployed and its address exists in `addresses.json` - tests will be run against that deployment. If there is no module in json - a new local instance of the module will be deployed. Remove the module key from `addresses.json` if you want to force-test a local module deployment.
...
4. If only verification is needed for an existing deployed contract, use the `--verify-only` flag followed by ABI-Encoded constructor args.

## Deployment addresses in `addresses.json`

The `addresses.json` file in root contains all existing deployed contracts on all of the target environments (mainnet/testnet/sandbox) on corresponding chains.
